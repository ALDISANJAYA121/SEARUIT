            ...
            <div class="rounded-lg border border-neutral-200 p-2 hit-button" data-id=${element.id}>
                <a href="${element.link}" target="_blank">
                    <div class="flex flex-row justify-between items-center">
                        <img src="img/${element.logo}" alt="" class="rounded-lg h-10">
                        <span class="${element.akses == "publik" ? "border-[#43a4d4]" : "border-[#e7a861]"} border text-[#282626] rounded-xl text-[10px] flex items-center justify-center px-2">${element.akses}</span>
                    </div>
                    <div class="flex flex-row justify-between items-center">
...
function search_bps_aceh(res){
    ...
    if(res.length <= 0){
        ...
        bps_aceh += 
        `
            <p id="not-found" class="text-gray-500 col-span-full">Tidak ditemukan di aplikasi BPS Provinsi Aceh</p>   
        `;
    } else {
        $('#res_bps_aceh').html(`
            <p class="text-gray-500 mb-1">Ditemukan ${res.length} aplikasi BPS Provinsi Aceh</p>
            `);
        res.forEach(element => {
            bps_aceh += 
            `
...
function search_bps_kabkot(res){
    ...
    if(res.length <= 0){
        ...
        bps_kabkot += 
        `
            <p id="not-found" class="text-gray-500 col-span-full">Tidak ditemukan di aplikasi BPS Kabupaten/Kota se-Provinsi Aceh</p>   
        `;
    } else {
        $('#res_bps_kabkot').html(`
            <p class="text-gray-500 mb-1">Ditemukan ${res.length} aplikasi BPS BPS Kabupaten/Kota se-Provinsi Aceh</p>
            `);
        res.forEach(element => {
            bps_kabkot += 
            `
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setupToggle('toggle-chevron-aceh', 'bps_aceh', false);
    setupToggle('toggle-chevron-kab', 'bps_kabkot', true);
    
    $('#search_0').on('input', function(){
        setupToggle('toggle-chevron', 'bps_ri', true);
        setupToggle('toggle-chevron-aceh', 'bps_aceh', true);
        setupToggle('toggle-chevron-kab', 'bps_kabkot', true);
    });
    $('#search_1').on('input', function(){
        setupToggle('toggle-chevron', 'bps_ri', true);
